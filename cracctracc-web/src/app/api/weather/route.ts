...
export async function GET() {
  return NextResponse.json({
    ...
    description: 'POST coordinates and timestamp to get historical weather data',
    parameters: {
...
async function fetchOpenMeteoWeather(lat: number, lon: number, timestamp: number): Promise<WeatherData | null> {
  try {
...
function generateMockWeatherData(lat: number, lon: number, timestamp: number): WeatherData {
  ...
  const windDirection = (baseDirection + 
  ...
  return {
    ...
    temperature: Math.round((20 - month * 2 + Math.sin(hour * Math.PI / 12) * 5 + 
      (Math.random() - 0.5) * 6) * 10) / 10,
...
export function cleanupWeatherCache(): void {
  const now = Date.now();
